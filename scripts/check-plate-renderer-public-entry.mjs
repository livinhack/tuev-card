...
assert(nonCommentLines.at(-1) === '} from "./lab-renderer-adapter.js?v=b348";', "renderer.js must delegate only to the b348 Card/Lab adapter cache boundary.");
...
assert(cardEntry.includes('from "./plate/renderer.js?v=b348"'), "Card entry must consume the b348 public renderer cache boundary.");
...
if (!process.exitCode) {
  console.log("Plate renderer public entry cleanup OK: renderer.js is a thin b348 public boundary with the established Card API only.");
}
