...
assert(nonCommentLines.at(-1) === '} from "./lab-renderer-adapter.js?v=b335";', "renderer.js must delegate only to the b335 Card/Lab adapter cache boundary.");
...
assert(cardEntry.includes('from "./plate/renderer.js?v=b335"'), "Card entry must consume the b335 public renderer cache boundary.");
...
if (!process.exitCode) {
  console.log("Plate renderer public entry cleanup OK: renderer.js is a thin b335 public boundary with the established Card API only.");
}
