...
assert(nonCommentLines.at(-1) === '} from "./lab-renderer-adapter.js?v=b347";', "renderer.js must delegate only to the b347 Card/Lab adapter cache boundary.");
...
assert(cardEntry.includes('from "./plate/renderer.js?v=b347"'), "Card entry must consume the b347 public renderer cache boundary.");
...
if (!process.exitCode) {
  console.log("Plate renderer public entry cleanup OK: renderer.js is a thin b347 public boundary with the established Card API only.");
}
