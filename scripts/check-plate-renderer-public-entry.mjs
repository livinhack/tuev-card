...
assert(nonCommentLines.at(-1) === '} from "./lab-renderer-adapter.js?v=b352";', "renderer.js must delegate only to the b352 Card/Lab adapter cache boundary.");
...
assert(cardEntry.includes('from "./plate/renderer.js?v=b352"'), "Card entry must consume the b352 public renderer cache boundary.");
assert(editor.includes('from "../plate/renderer.js?v=b352"'), "Editor must consume the b352 public renderer cache boundary.");
...
if (!process.exitCode) {
  console.log("Plate renderer public entry cleanup OK: renderer.js is a thin b352 public boundary with the established Card API only.");
}
