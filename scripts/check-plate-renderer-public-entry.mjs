...
assert(nonCommentLines.at(-1) === '} from "./lab-renderer-adapter.js?v=b339";', "renderer.js must delegate only to the b339 Card/Lab adapter cache boundary.");
...
assert(cardEntry.includes('from "./plate/renderer.js?v=b339"'), "Card entry must consume the b339 public renderer cache boundary.");
...
if (!process.exitCode) {
  console.log("Plate renderer public entry cleanup OK: renderer.js is a thin b339 public boundary with the established Card API only.");
}
