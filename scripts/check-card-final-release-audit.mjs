...
function fail(message) {
  console.error(`b347 final release audit check failed: ${message}`);
  process.exitCode = 1;
}
...
assert(readme.includes("b347") && /Final Release Audit/i.test(readme), "README must identify b347 as Final Release Audit");
assert(handover.includes("b347") && /Final Release Audit/i.test(handover), "HANDOVER must identify b347 as Final Release Audit");
...
assert(/keine Kennzeichen-Geometrie/i.test(readme) && /keine Reminder-Integration/i.test(readme), "README must explicitly state the b347 non-goals");

assert(card.includes('from "./plate/renderer.js?v=b347"'), "Card runtime must use b347 public renderer cache marker");
assert(editor.includes('from "../plate/renderer.js?v=b347"'), "Editor must use b347 public renderer cache marker");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b347"'), "Public renderer must use b347 adapter cache marker");
assert(card.includes('from "./utils/html-escape.js?v=b347"'), "Card entry must keep shared HTML escaping with b347 cache marker");
assert(editor.includes('from "../utils/html-escape.js?v=b347"'), "Editor must keep shared HTML escaping with b347 cache marker");
...
assert(dist.includes("b347"), "dist bundle must be rebuilt with b347 markers");

if (!process.exitCode) {
  console.log("b347 final release audit OK: docs, cache markers, font notes, security cleanup, and later Reminder boundary are release-ready.");
}
