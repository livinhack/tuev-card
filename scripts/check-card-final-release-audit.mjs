...
function fail(message) {
  console.error(`b352 final release audit check failed: ${message}`);
  process.exitCode = 1;
}
...
assert(readme.includes("b352") && /Final Release Audit/i.test(readme), "README must identify b352 as Final Release Audit");
assert(handover.includes("b352") && /Final Release Audit/i.test(handover), "HANDOVER must identify b352 as Final Release Audit");
...
assert(/keine Kennzeichen-Geometrie/i.test(readme) && /keine Reminder-Integration/i.test(readme), "README must explicitly state the b352 non-goals");

assert(card.includes('from "./plate/renderer.js?v=b352"'), "Card runtime must use b352 public renderer cache marker");
assert(editor.includes('from "../plate/renderer.js?v=b352"'), "Editor must use b352 public renderer cache marker");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b352"'), "Public renderer must use b352 adapter cache marker");
assert(card.includes('from "./utils/html-escape.js?v=b352"'), "Card entry must keep shared HTML escaping with b352 cache marker");
assert(editor.includes('from "../utils/html-escape.js?v=b352"'), "Editor must keep shared HTML escaping with b352 cache marker");
...
assert(dist.includes("b352"), "dist bundle must be rebuilt with b352 markers");

if (!process.exitCode) {
  console.log("b352 final release audit OK: docs, cache markers, font notes, security cleanup, and later Reminder boundary are release-ready.");
}
