...
function fail(message) {
  console.error(`b355 final release audit check failed: ${message}`);
  process.exitCode = 1;
}
...
assert(readme.includes("b355") && /Final Release Audit/i.test(readme), "README must identify b355 as Final Release Audit");
assert(handover.includes("b355") && /Final Release Audit/i.test(handover), "HANDOVER must identify b355 as Final Release Audit");
...
assert(/keine Kennzeichen-Geometrie/i.test(readme) && /keine Reminder-Integration/i.test(readme), "README must explicitly state the b355 non-goals");

assert(card.includes('from "./plate/renderer.js?v=b355"'), "Card runtime must use b355 public renderer cache marker");
assert(editor.includes('from "../plate/renderer.js?v=b355"'), "Editor must use b355 public renderer cache marker");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b355"'), "Public renderer must use b355 adapter cache marker");
assert(card.includes('from "./utils/html-escape.js?v=b355"'), "Card entry must keep shared HTML escaping with b355 cache marker");
assert(editor.includes('from "../utils/html-escape.js?v=b355"'), "Editor must keep shared HTML escaping with b355 cache marker");
...
assert(dist.includes("b355"), "dist bundle must be rebuilt with b355 markers");

if (!process.exitCode) {
  console.log("b355 final release audit OK: docs, cache markers, font notes, security cleanup, and later Reminder boundary are release-ready.");
}
