...
assert(card.includes('./plate/renderer.js?v=b342'), "Card runtime must use the b342 public renderer cache marker.");
assert(editor.includes('../plate/renderer.js?v=b342'), "Editor must use the b342 public renderer cache marker.");
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b342'), "Public renderer entry must delegate to the b342 adapter cache marker.");
...
assert(readme.includes("b342"), "README must identify the b342 checkpoint.");
assert(handover.includes("b342"), "HANDOVER must identify the b342 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b342 cache markers are current, and release docs keep the font note.`);
