...
assert(card.includes('./plate/renderer.js?v=b338'), "Card runtime must use the b338 public renderer cache marker.");
assert(editor.includes('../plate/renderer.js?v=b338'), "Editor must use the b338 public renderer cache marker.");
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b338'), "Public renderer entry must delegate to the b338 adapter cache marker.");
...
assert(readme.includes("b338"), "README must identify the b338 checkpoint.");
assert(handover.includes("b338"), "HANDOVER must identify the b338 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b338 cache markers are current, and release docs keep the font note.`);
