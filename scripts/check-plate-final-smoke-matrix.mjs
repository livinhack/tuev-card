...
assert(card.includes('./plate/renderer.js?v=b350'), "Card runtime must use the b350 public renderer cache marker.");
assert(editor.includes('../plate/renderer.js?v=b350'), "Editor must use the b350 public renderer cache marker.");
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b350'), "Public renderer entry must delegate to the b350 adapter cache marker.");
...
assert(readme.includes("b350"), "README must identify the b350 checkpoint.");
assert(handover.includes("b350"), "HANDOVER must identify the b350 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b350 cache markers are current, and release docs keep the font note.`);
