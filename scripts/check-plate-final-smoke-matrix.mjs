...
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b347'), "Public renderer entry must delegate to the b347 adapter cache marker.");
...
assert(readme.includes("b347"), "README must identify the b347 checkpoint.");
assert(handover.includes("b347"), "HANDOVER must identify the b347 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b347 cache markers are current, and release docs keep the font note.`);
