...
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b346'), "Public renderer entry must delegate to the b346 adapter cache marker.");
...
assert(readme.includes("b346"), "README must identify the b346 checkpoint.");
assert(handover.includes("b346"), "HANDOVER must identify the b346 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b346 cache markers are current, and release docs keep the font note.`);
