...
assert(card.includes('./plate/renderer.js?v=b343'), "Card runtime must use the b343 public renderer cache marker.");
assert(editor.includes('../plate/renderer.js?v=b343'), "Editor must use the b343 public renderer cache marker.");
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b343'), "Public renderer entry must delegate to the b343 adapter cache marker.");
...
assert(readme.includes("b343"), "README must identify the b343 checkpoint.");
assert(handover.includes("b343"), "HANDOVER must identify the b343 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b343 cache markers are current, and release docs keep the font note.`);
