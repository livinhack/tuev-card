...
assert(card.includes('./plate/renderer.js?v=b337'), "Card runtime must use the b337 public renderer cache marker.");
assert(editor.includes('../plate/renderer.js?v=b337'), "Editor must use the b337 public renderer cache marker.");
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b337'), "Public renderer entry must delegate to the b337 adapter cache marker.");
...
assert(readme.includes("b337"), "README must identify the b337 checkpoint.");
assert(handover.includes("b337"), "HANDOVER must identify the b337 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b337 cache markers are current, and release docs keep the font note.`);
