...
assert(card.includes('./plate/renderer.js?v=b351'), "Card runtime must use the b351 public renderer cache marker.");
assert(editor.includes('../plate/renderer.js?v=b351'), "Editor must use the b351 public renderer cache marker.");
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b351'), "Public renderer entry must delegate to the b351 adapter cache marker.");
...
assert(readme.includes("b351"), "README must identify the b351 checkpoint.");
assert(handover.includes("b351"), "HANDOVER must identify the b351 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b351 cache markers are current, and release docs keep the font note.`);
