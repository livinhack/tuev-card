...
assert(publicRenderer.includes('./lab-renderer-adapter.js?v=b339'), "Public renderer entry must delegate to the b339 adapter cache marker.");
...
assert(readme.includes("b339"), "README must identify the b339 checkpoint.");
assert(handover.includes("b339"), "HANDOVER must identify the b339 checkpoint.");
...
if (!process.exitCode) console.log(`Final card plate smoke matrix OK: ${cases.length} core Card/Lab plate variants render, b339 cache markers are current, and release docs keep the font note.`);
