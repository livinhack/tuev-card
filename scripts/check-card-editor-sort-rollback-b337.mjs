...
function assert(condition, message) {
  if (!condition) {
    console.error(`b353 sort rollback check failed: ${message}`);
    process.exit(1);
  }
}
...
assert(editor.includes('from "../plate/renderer.js?v=b353"'), "editor cache marker must be b353");
assert(card.includes('from "./plate/renderer.js?v=b353"'), "card cache marker must be b353");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b353"'), "public renderer cache marker must be b353");
assert(groups.includes('./entities.js?v=b353'), "groups helper cache marker must be b353");

assert(!editor.includes('getSortedUngroupedDraftEntityIds'), "b338/b353 visible ungrouped draft re-sort helper must be removed");
...
console.log('✓ b353 sort rollback to b337 with b353 font/frame fixes preserved');
