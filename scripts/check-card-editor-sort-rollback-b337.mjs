...
function assert(condition, message) {
  if (!condition) {
    console.error(`b340 sort rollback check failed: ${message}`);
    process.exit(1);
  }
}
...
assert(editor.includes('from "../plate/renderer.js?v=b340"'), "editor cache marker must be b340");
assert(card.includes('from "./plate/renderer.js?v=b340"'), "card cache marker must be b340");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b340"'), "public renderer cache marker must be b340");
assert(groups.includes('./entities.js?v=b340'), "groups helper cache marker must be b340");

assert(!editor.includes('getSortedUngroupedDraftEntityIds'), "b338/b340 visible ungrouped draft re-sort helper must be removed");
...
console.log('✓ b340 sort rollback to b337 with b340 font/frame fixes preserved');
