...
assert(card.includes('getPreviewScaleSafetyPx()'), 'b355 must name the preview scale safety inset.');
...
assert(!card.includes('renderPreviewDiagnostics'), 'temporary b352/b353 preview diagnostic renderer must be removed in b355 cleanup.');
assert(!card.includes('data-preview-layout-diagnostics'), 'temporary preview diagnostic overlay must not be visible in b355 cleanup.');
assert(card.includes('previewScaleSafetyPx'), 'b355 must keep the named safety inset in the layout context.');
...
if (!process.exitCode) {
  console.log('Card editor preview scale cleanup OK: b355 keeps the inner-width scale fix and removes the temporary diagnostic overlay.');
}
