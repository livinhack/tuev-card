...
assert(card.includes('from "./plate/renderer.js?v=b341"'), "Card must use the b341 renderer cache marker.");
assert(editor.includes('from "../plate/renderer.js?v=b341"'), "Editor must use the b341 renderer cache marker.");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b341"'), "Public renderer must use the b341 adapter cache marker.");
...
assert(editor.includes('getGroupAccentColor') && editor.includes('../card/groups.js?v=b341'), "Editor must import the group color helper through the b341 groups boundary.");
assert(groups.includes('./entities.js?v=b341'), "Groups helper must use the b341 entities cache marker.");

assert(readme.includes("b341") && /Kennzeichen grafisch darstellen/i.test(readme), "README must document the b341 editor option fix.");
assert(handover.includes("b341") && /Sortier/i.test(handover) && /Farben/i.test(handover), "HANDOVER must document b341 sort and color fixes.");
