...
assert(card.includes('from "./plate/renderer.js?v=b350"'), "Card must use the b350 renderer cache marker.");
assert(editor.includes('from "../plate/renderer.js?v=b350"'), "Editor must use the b350 renderer cache marker.");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b350"'), "Public renderer must use the b350 adapter cache marker.");
...
assert(editor.includes('getGroupAccentColor') && editor.includes('../card/groups.js?v=b350'), "Editor must import the group color helper through the b350 groups boundary.");
assert(groups.includes('./entities.js?v=b350'), "Groups helper must use the b350 entities cache marker.");

assert(readme.includes("b350") && /Kennzeichen grafisch darstellen/i.test(readme), "README must document the b350 editor option fix.");
assert(handover.includes("b350") && /Sortier/i.test(handover) && /Farben/i.test(handover), "HANDOVER must document b350 sort and color fixes.");
