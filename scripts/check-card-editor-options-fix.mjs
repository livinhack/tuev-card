...
assert(card.includes('from "./plate/renderer.js?v=b351"'), "Card must use the b351 renderer cache marker.");
assert(editor.includes('from "../plate/renderer.js?v=b351"'), "Editor must use the b351 renderer cache marker.");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b351"'), "Public renderer must use the b351 adapter cache marker.");
...
assert(editor.includes('getGroupAccentColor') && editor.includes('../card/groups.js?v=b351'), "Editor must import the group color helper through the b351 groups boundary.");
assert(groups.includes('./entities.js?v=b351'), "Groups helper must use the b351 entities cache marker.");

assert(readme.includes("b351") && /Kennzeichen grafisch darstellen/i.test(readme), "README must document the b351 editor option fix.");
assert(handover.includes("b351") && /Sortier/i.test(handover) && /Farben/i.test(handover), "HANDOVER must document b351 sort and color fixes.");
