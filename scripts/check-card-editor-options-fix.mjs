...
assert(card.includes('from "./plate/renderer.js?v=b342"'), "Card must use the b342 renderer cache marker.");
assert(editor.includes('from "../plate/renderer.js?v=b342"'), "Editor must use the b342 renderer cache marker.");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b342"'), "Public renderer must use the b342 adapter cache marker.");
...
assert(editor.includes('getGroupAccentColor') && editor.includes('../card/groups.js?v=b342'), "Editor must import the group color helper through the b342 groups boundary.");
assert(groups.includes('./entities.js?v=b342'), "Groups helper must use the b342 entities cache marker.");

assert(readme.includes("b342") && /Kennzeichen grafisch darstellen/i.test(readme), "README must document the b342 editor option fix.");
assert(handover.includes("b342") && /Sortier/i.test(handover) && /Farben/i.test(handover), "HANDOVER must document b342 sort and color fixes.");
