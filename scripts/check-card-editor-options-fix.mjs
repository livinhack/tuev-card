...
assert(card.includes('from "./plate/renderer.js?v=b348"'), "Card must use the b348 renderer cache marker.");
assert(editor.includes('from "../plate/renderer.js?v=b348"'), "Editor must use the b348 renderer cache marker.");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b348"'), "Public renderer must use the b348 adapter cache marker.");
...
assert(editor.includes('getGroupAccentColor') && editor.includes('../card/groups.js?v=b348'), "Editor must import the group color helper through the b348 groups boundary.");
assert(groups.includes('./entities.js?v=b348'), "Groups helper must use the b348 entities cache marker.");

assert(readme.includes("b348") && /Kennzeichen grafisch darstellen/i.test(readme), "README must document the b348 editor option fix.");
assert(handover.includes("b348") && /Sortier/i.test(handover) && /Farben/i.test(handover), "HANDOVER must document b348 sort and color fixes.");
