...
assert(card.includes('from "./plate/renderer.js?v=b349"'), "Card must use the b349 renderer cache marker.");
assert(editor.includes('from "../plate/renderer.js?v=b349"'), "Editor must use the b349 renderer cache marker.");
assert(renderer.includes('from "./lab-renderer-adapter.js?v=b349"'), "Public renderer must use the b349 adapter cache marker.");
...
assert(editor.includes('getGroupAccentColor') && editor.includes('../card/groups.js?v=b349'), "Editor must import the group color helper through the b349 groups boundary.");
assert(groups.includes('./entities.js?v=b349'), "Groups helper must use the b349 entities cache marker.");

assert(readme.includes("b349") && /Kennzeichen grafisch darstellen/i.test(readme), "README must document the b349 editor option fix.");
assert(handover.includes("b349") && /Sortier/i.test(handover) && /Farben/i.test(handover), "HANDOVER must document b349 sort and color fixes.");
