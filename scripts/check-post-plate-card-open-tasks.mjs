...
assert(readme.includes("b344"), "README must identify the current b344 stand.");
assert(handover.includes("b344"), "HANDOVER must identify the current b344 stand.");
...
assert(renderer.includes('./lab-renderer-adapter.js?v=b344'), "Public renderer entry must delegate to the b344 adapter cache marker.");
...
if (!process.exitCode) {
  console.log("Post-plate Card open-tasks audit OK: b344 markers, frozen/prepared plate status, remaining Card buckets, font note, and later Reminder integration are documented.");
}
