...
assert(readme.includes("b348"), "README must identify the current b348 stand.");
assert(handover.includes("b348"), "HANDOVER must identify the current b348 stand.");
...
assert(renderer.includes('./lab-renderer-adapter.js?v=b348'), "Public renderer entry must delegate to the b348 adapter cache marker.");
...
if (!process.exitCode) {
  console.log("Post-plate Card open-tasks audit OK: b348 markers, frozen/prepared plate status, remaining Card buckets, font note, and later Reminder integration are documented.");
}
