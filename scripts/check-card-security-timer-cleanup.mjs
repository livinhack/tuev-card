...
function fail(message) {
  console.error(`b343 security/timer cleanup check failed: ${message}`);
  process.exitCode = 1;
}
...
assert(entry.includes('import { escapeHtml } from "./utils/html-escape.js?v=b343"'), "Card entry must import shared HTML escape helper");
assert(cardParts.includes('import { escapeHtml } from "../utils/html-escape.js?v=b343"'), "Card render-parts must import shared HTML escape helper");
assert(editor.includes('import { escapeHtml } from "../utils/html-escape.js?v=b343"'), "Editor must import shared HTML escape helper instead of owning another HTML escape formula");
...
assert(pkg.includes('"version": "0.1.1-b343"'), "package version must be b343");

if (!process.exitCode) {
  console.log("b343 security/timer cleanup OK: dead font timer removed, HTML escaping shared, confirm timers named/managed, adapter SVG escaping reused.");
}
