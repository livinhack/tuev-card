...
function fail(message) {
  console.error(`b350 preview/popup stability check failed: ${message}`);
  process.exit(1);
}
...
assert(pkg.includes('check:card-editor-preview-popup-stability'), "package must expose the b350 preview/popup stability check");
assert(pkg.includes('npm run check:card-editor-preview-popup-stability'), "main check script must execute the b350 stability check");

assert(entry.includes('option changes only the vehicle content; it must not change the'), "Card must document the b350 text/graphic preview column stability rule");
...
assert(layoutContextBlock.includes('visiblePreviewWidth >= simulatedWidth - 4'), "Scale bypass must use the actually visible preview width, not measuredWidth");
...
assert(!editor.includes('document.addEventListener("pointerdown", this._boundHandleDocumentPointerDown, true);'), "Editor must not use pointerdown capture for floating-panel outside close after b350 rollback");
...
console.log("b350 preview/popup stability OK: text/graphic preview columns share one simulation path, dashboard-edit width fallback is guarded, and floating panels use the b344 deferred click-close path while text preview stability remains.");
