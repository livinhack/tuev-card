...
function fail(message) {
  console.error(`b346 preview/popup stability check failed: ${message}`);
  process.exit(1);
}
...
assert(pkg.includes('check:card-editor-preview-popup-stability'), "package must expose the b346 preview/popup stability check");
assert(pkg.includes('npm run check:card-editor-preview-popup-stability'), "main check script must execute the b346 stability check");

assert(entry.includes('option changes only the vehicle content; it must not change the'), "Card must document the b346 text/graphic preview column stability rule");
assert(!entry.includes('this.config?.plate_style !== "plate"'), "Text plate mode must no longer bypass the editor preview column simulation");
...
console.log("b346 preview/popup stability OK: text/graphic preview columns share one simulation path, dashboard-edit width fallback is guarded, and floating panels close reliably on first outside interaction.");
