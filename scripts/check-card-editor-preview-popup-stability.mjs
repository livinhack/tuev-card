...
function fail(message) {
  console.error(`b355 preview/popup stability check failed: ${message}`);
  process.exit(1);
}
...
assert(pkg.includes('check:card-editor-preview-popup-stability'), "package must expose the b355 preview/popup stability check");
assert(pkg.includes('npm run check:card-editor-preview-popup-stability'), "main check script must execute the b355 stability check");

assert(entry.includes('option changes only the vehicle content; it must not change the'), "Card must document the b355 text/graphic preview column stability rule");
...
assert(!editor.includes('document.addEventListener("pointerdown", this._boundHandleDocumentPointerDown, true);'), "Editor must not use pointerdown capture for floating-panel outside close after b355 rollback");
...
console.log("b355 preview/popup stability OK: text/graphic preview columns share one simulation path, dashboard-edit width fallback is guarded, and floating panels use the b344 deferred click-close path while text preview stability remains.");
