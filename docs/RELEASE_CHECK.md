...
Current checked version: `b58`.

## b58 release note

`b58` is the first semantic test release. It is based on the confirmed release-candidate and repository-cleanup checkpoints and should not introduce new runtime behavior compared with the tested candidate state.
...
```text
/local/community/tuev-card/tuev-card.js?v=b58
```
...
/hacsfiles/tuev-card/tuev-card.js?v=b58
```
...
`b58` tunes only the background/contrast and rotation of the stamp-style confirmation overlay for `show_badge: false`. The b55 frame and typography concept remain unchanged.

## b58 stamp animation note
