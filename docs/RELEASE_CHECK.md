...
`b59` is the first semantic test release. It is based on the confirmed release-candidate and repository-cleanup checkpoints and should not introduce new runtime behavior compared with the tested candidate state.
...
```text
/local/community/tuev-card/tuev-card.js?v=b59
```
...
/hacsfiles/tuev-card/tuev-card.js?v=b59
```
...
`b59` tunes only the background/contrast and rotation of the stamp-style confirmation overlay for `show_badge: false`. The b55 frame and typography concept remain unchanged.

## b59 stamp animation note
...
`b59` fixes the stamp confirmation click/service flow after the b58 animation work. It also removes a small obsolete wrapper in the delayed confirmation path. No layout, renderer, editor or EuroPlate behavior was intentionally changed.
